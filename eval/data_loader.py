...
def load_data(data_name, split, data_dir="./data"):
    ...
    if os.path.exists(data_file):
        ...
        if data_name == "math":
            ...
            #         examples.extend(sub_examples)
            # dataset = Dataset.from_list(examples)
            with open(f"{data_dir}/bbh/bbh.json", "r") as f:
                dataset = json.load(f)
            ...
            dataset = load_dataset(
                "json", data_files=f"{data_dir}/hungarian_exam/{split}.jsonl"
            )
        elif data_name == "carp_en":
            dataset = load_jsonl(f"{data_dir}/carp_en/test.jsonl")
        elif data_name == "math_collection":
            dataset = datasets.load_from_disk(data_dir)
        elif data_name == "arc":
            with open(f"{data_dir}/arc/challenge.json") as f:
                examples = json.load(f)
                dataset = Dataset.from_list(examples)
        elif "gpqa" in data_name:
            dataset = load_jsonl(f"{data_dir}/gpqa/{data_name}-mc.jsonl")
        else:
